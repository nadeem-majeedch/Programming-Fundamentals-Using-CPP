...
**Module 15 · Lectures L29–L30 · Week 15 · 120 minutes · guided/independent 40/60**

Prerequisites: completion of Labs 1–14 (Lab 1: none). Bring your laptop
...
- Read/write text files with ifstream/ofstream
- Use getline + istringstream for robust line parsing
- Check stream state and classify failures
- Contrast stream-state error handling with exceptions (try/catch demo)
...
ifstream/ofstream, line parsing, failure taxonomy, exception demo — full treatment in the
...
1. Which open failure is fatal, and which data line is a skip?
2. What does !in test after opening?
3. When would try/catch be preferable to return codes?

## 6. Instructor demonstration (15 min)

The instructor or TA demonstrates: **Streams, robust reading, error strategies**. Watch for the *workflow*
...
1. guided: write-then-read-back a 5-number file
2. guided: CSV line parsing with skip-and-report
3. independent: sensor-log to summary-report pipeline
4. independent: append-mode grade book with read-back report
...
**Robustness clinic: 3 fault injections (missing file, bad line, empty file)**
...
reliably read/write text files with ifstream/ofstream; use getline + istringstream for robust line parsing; check stream state and classify failures; contrast stream-state error handling with exceptions (try/catch demo). This maps to the module's PF outcomes and feeds
