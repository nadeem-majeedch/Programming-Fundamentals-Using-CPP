...
Prerequisites: completion of Labs 1–9 (Lab 1: none). Bring your laptop
...
nested loops, row/col aggregations, transpose — full treatment in the
...
1. Which loop order prints row-major?
2. For the main diagonal, what relation binds r and c?
3. Why must the transpose swap only j > i?

## 6. Instructor demonstration (15 min)

The instructor or TA demonstrates: **Grid traversal, row/column logic**. Watch for the *workflow*
...
1. guided: grid fill + row-wise and column-wise prints
2. guided: row/col totals with per-row accumulators
3. independent: matrix addition with dimension checks
4. independent: transpose + main/anti-diagonal program
...
reliably traverse a grid row-major and column-major; compute row/column totals and diagonals; transpose a square matrix in place; pass 2-D arrays to functions with the column dimension fixed. This maps to the module's PF outcomes and feeds
