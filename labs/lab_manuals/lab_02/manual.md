# Lab 2 · Variables and Calculations

**Module 2 · Lectures L03–L04 · Week 2 · 120 minutes · guided/independent 70/30**
...
- Choose int vs double deliberately for money vs counts
- Format output with fixed precision and field widths
- Read values with cin and validate non-negative input
...
type selection, the integer-division trap, formatting — full treatment in the
...
1. Why does 7/2 give 3 but 7.0/2 gives 3.5?
2. Which type would you store a phone number in, and why?
3. What does setprecision(2) do WITHOUT fixed?

## 6. Instructor demonstration (15 min)

The instructor or TA demonstrates: **Types, initialization, formatted I/O**. Watch for the *workflow*
...
1. guided: types-and-sizes tour (sizeof, limits)
2. guided: receipt line with setw/setprecision
3. independent: unit-price calculator with 2-dp totals
4. independent: currency converter with a named rate constant
...
**A calculator whose totals silently truncate — diagnose, then fix**
...
reliably declare and initialize variables of appropriate types; choose int vs double deliberately for money vs counts; format output with fixed precision and field widths; read values with cin and validate non-negative input. This maps to the module's PF outcomes and feeds
