...
**Module 5 · Lectures L09–L10 · Week 5 · 120 minutes · guided/independent 60/40**

Prerequisites: completion of Labs 1–4 (Lab 1: none). Bring your laptop
...
- Choose loop form deliberately (count known vs unknown)
- Write sentinel loops with the priming read
- Accumulate sum/min/max/count in one pass
- Use break/continue without breaking structure
...
loop form selection, sentinel discipline, accumulators — full treatment in the
...
1. Where does the priming read go in a sentinel loop?
2. for (int i = 0; i < n; ++i) — how many iterations?
3. What repeats forever in while (i < 10) if the update is missing?

## 6. Instructor demonstration (15 min)

The instructor or TA demonstrates: **while/do-while/for, sentinels, accumulation**. Watch for the *workflow*
...
1. guided: three-forms-of-one-loop conversion exercise
2. guided: running max/min/count pass over inputs
3. independent: sentinel-driven statistics reporter
4. independent: multiplication table grid with nested loops
...
**A missing-update infinite loop — spot it BEFORE running (then run)**
...
reliably choose loop form deliberately (count known vs unknown); write sentinel loops with the priming read; accumulate sum/min/max/count in one pass; use break/continue without breaking structure. This maps to the module's PF outcomes and feeds
